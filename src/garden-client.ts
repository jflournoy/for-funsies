...
import { hash32, unit, mulberry32, hueFor, type GardenSnapshot } from "./garden-core.js";
 ...
 */
export function renderGardenDom(snap: GardenSnapshot): { svg: SVGSVGElement; tooltip: HTMLDivElement } {
  const W = 800;
  const H = 360;
  const CX = W / 2;
  const CY = H / 2;
  ...
  txt.setAttribute("font-family", "ui-monospace,'SF Mono',Menlo,Consolas,monospace");
  ...
  const starElements: { el: SVGCircleElement; commit: { hash: string; author: string; date: string } }[] = [];
  ...
  for (let i = 0; i < maxStars; i++) {
    ...
    starElements.push({ el: circle, commit: { hash: c.hash, author: c.author, date: c.date } });
  }
  ...
  tooltip.style.cssText = "position:fixed;pointer-events:none;z-index:1000;display:none;background:var(--bg,#14140f);color:var(--fg,#ece7da);border:1px solid var(--rule,#2e2c25);border-radius:6px;padding:6px 10px;font:12px/1.5 ui-monospace,'SF Mono',Menlo,Consolas,monospace;box-shadow:0 2px 8px rgba(0,0,0,0.3);";
...
export function initGarden(): void {
  ...
  try {
    const snap = JSON.parse(dataEl.textContent ?? "null") as GardenSnapshot | null;
    if (!snap) return;
